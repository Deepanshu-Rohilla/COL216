...
void printChange_in_Each_Cycle(int ROW_ACCESS_DELAY,int COL_ACCESS_DELAY,int type,int instruction_Num,int numberOfClockCycles,int addr, string inst_name,string Register_mod,string Memory ,string DARM_Activity){
    // to print all possible changes in each cycle

   
    if(instruction_Num!=8 && instruction_Num!=9){
        cout<<"Cycle number "<<numberOfClockCycles<<": ";
       cout<<"Instruction Address: "<<addr<<", Executed Instruction : "+ inst_name+" \n" +"Modified Register : "+Register_mod+" \n"+"Modified Memory Location : " + Memory + "\n"+"Activity on DRAM : " +DARM_Activity+ " .";
    }
    ...
    // else{
        ...
        // row =(DRAM[(i+2)/1024][(i+2)%1024] + registers[DRAM[(i+3)/1024][(i+3)%1024]])/1024;
            ...
            if(choice){
              ...
              printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,2,8,numberOfClockCycles,i,"Nothing","Nothing","Nothing","Loading row " +to_string(row)+ " into row-buffer");
            }
            ...
            if(choice){
              ...
              printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,1,8,numberOfClockCycles,i,"Nothing",Register_map[DRAM[(i+1)/1024][(i+1)%1024]] +" = "+to_string(Row_Buffer[col]) ,"Nothing","DRAM column access and write value in register");
            }else{
              numberOfClockCycles+=excess;
              printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,1,8,numberOfClockCycles,i,"Nothing",Register_map[DRAM[(i+1)/1024][(i+1)%1024]] +" = "+to_string(Row_Buffer[col]) ,"Nothing","Loading row " +to_string(row)+ " into row-buffer "+" :-> "+" DRAM column access and write value in register");
              choice=true;
             // counter=0;
            }
        }
            ...
            if(row==(DRAM[(i+2)/1024][(i+2)%1024] + registers[DRAM[(i+3)/1024][(i+3)%1024]])/1024){
                ...
                if(choice){
                  numberOfClockCycles+=ROW_ACCESS_DELAY;
                  printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,2,8,numberOfClockCycles,i,"Nothing","Nothing","Nothing","Write-back row: "+to_string(row));
                }
                row=(DRAM[(i+2)/1024][(i+2)%1024] + registers[DRAM[(i+3)/1024][(i+3)%1024]])/1024;
                load_data(Row_Buffer,DRAM,row);
                ...
                if(choice){
                    ...
                    numberOfClockCycles+=excess;
                    printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,1,8,numberOfClockCycles,i,"Nothing",Register_map[DRAM[(i+1)/1024][(i+1)%1024]] +" = "+to_string(Row_Buffer[col]),"Nothing ","Write-back row: "+to_string(row)+ " :-> "+" Loading row " +to_string(row)+ " into row-buffer "+" :-> "+"DRAM column access and write value in register");
                    choice=true;
                   // counter=0;
                }
            }
          ...
          i = v[4];
           ...
           i =idx_num;
            ...
            if(row==-1){    // When first time "sw" is called (means when row_buffer is empty)
                ...
                if(choice){
                    numberOfClockCycles+=ROW_ACCESS_DELAY;
                    printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,2,9,numberOfClockCycles,i,"Nothing","Nothing","Nothing","Activate row: "+to_string(row));
                }
                ...
                if(choice){
                    numberOfClockCycles+=COL_ACCESS_DELAY;
                    printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,1,9,numberOfClockCycles,i,"Nothing","Nothing","Address "+to_string(memory_address)+" = "+to_string(Row_Buffer[col]),"DRAM column access and update");
                }else{
                    numberOfClockCycles+=excess;
                    printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,1,9,numberOfClockCycles,i,"Nothing","Nothing","Address "+to_string(memory_address)+" = "+to_string(Row_Buffer[col]),"Activate row: "+to_string(row)+" :-> "+"DRAM column access and update");
                    choice=true;
                    //counter=0;
                }
                // DRAM[row][col]=registers[DRAM[(i+1)/1024][(i+1)%1024]];
                       ...
                       /// row = rowNum;
                        ...
                        if(row==(DRAM[(i+2)/1024][(i+2)%1024]+ registers[DRAM[(i+3)/1024][(i+3)%1024]])/1024){  // it same row come and only column_offset requirr to modify data   
                            ...
                            if(choice){
                                numberOfClockCycles+=ROW_ACCESS_DELAY;
                                printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,2,9,numberOfClockCycles,i,"Nothing","Nothing","Nothing","Write-back row: "+to_string(row));
                            }
                            row=(DRAM[(i+2)/1024][(i+2)%1024] + registers[DRAM[(i+3)/1024][(i+3)%1024]])/1024;
                            load_data(Row_Buffer,DRAM,row);
                            ...
                            if(choice){
                                ...
                                numberOfClockCycles+=excess;
                                printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,1,9,numberOfClockCycles,i,"Nothing","Nothing","Address "+to_string(memory_address)+" = "+to_string(Row_Buffer[col]),"Write-back row: "+to_string(row) +" :->"+ "Activate row: "+to_string(row)+" :-> DRAM column access and update");
                                choice=true;
                                //counter=0;
                            }
                        }
...
void parallel_instruction(int val){
        ...
        if(counter>=delay_num){
            ...
            if(tmp1!=-1){
                ...
                counter=counter-(delay_num);
                idx_num=v_d[4];
                 if(v_d[0]==8){
                    print_lw(tmp1,(*it_copy));
                 }else{
                   print_sw(tmp1,(*it_copy));
                 }
                }
        }
            else if(val==1 && counter>0 && row!=-1){
                 int tmp1=-1;
                 for(int i=0;i<1024;i++){
                  if(!queueArray[i].empty()){
                      choice =false;
...
void Non_blocking_check(int val ){

     if(row!=-1 && check_counter()){

        if(!queueArray[row].empty()){
          if(counter>=COL_ACCESS_DELAY){
            while(counter>=COL_ACCESS_DELAY && !queueArray[row].empty()){
            choice=false;
            vector<int> v_d = queueArray[row].front();
            deque<vector<int>>q1 ;
            q1.push_back(v_d);
            counter=counter-COL_ACCESS_DELAY;
            excess=-counter;
            queueArray[row].pop_front();
            auto it_copy = q1.begin();
            idx_num=v_d[4];
           if(v_d[0]==8){
                print_lw(row,(*it_copy));
           }else{
                print_sw(row,(*it_copy));
            }
           }
          ...
          if(val==1 && counter>0){
            ...
            q1.push_back(v_d);
            queueArray[row].pop_front();
            excess=COL_ACCESS_DELAY-counter;
            counter=0;
            auto it_copy = q1.begin();
            idx_num=v_d[4];
           if(v_d[0]==8){
...
int main(int argc, char** argv){
    ...
    while(i<x ){     // In this loop we will execute the command of "Instruction Set" which is aleready stored in DRAM
        ...
        case 1:            //add
            ...
            printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,1,numberOfClockCycles,i,"add",Register_map[DRAM[(i+1)/1024][(i+1)%1024]]+" = "+ to_string(registers[DRAM[(i+1)/1024][(i+1)%1024]] ),"Nothing ","Nothing");
            i=i+4;
            break;
        ...
        case 2:           //sub
            ...
            //printRegisters(numberOfClockCycles);
            printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,2,numberOfClockCycles,i,"sub",Register_map[DRAM[(i+1)/1024][(i+1)%1024]]+" = "+ to_string(registers[DRAM[(i+1)/1024][(i+1)%1024]] ),"Nothing ","Nothing");
            i=i+4;
            break;
        ...
        case 3:      //mul
            ...
            //printRegisters(numberOfClockCycles);
            printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,3,numberOfClockCycles,i,"mul",Register_map[DRAM[(i+1)/1024][(i+1)%1024]]+" = "+ to_string(registers[DRAM[(i+1)/1024][(i+1)%1024]] ),"Nothing ","Nothing");
            i=i+4;
            break;
        ...
        case 4:   //beq
            ...
            if(registers[DRAM[(i+1)/1024][(i+1)%1024]] == registers[DRAM[(i+2)/1024][(i+2)%1024]]){
                printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,4,numberOfClockCycles,i,"beq","Nothing ","Nothing ","Nothing");
        ...
        case 5:   //bne
            ...
            //          if(counter==COL_ACCESS_DELAY){
            //             choice=false;
            //             vector<int> v_d = queueArray[i].front();
            //             deque<vector<int>>q1 ;
            //             q1.push_back(v_d);
            //             queueArray[i].pop_front();
            //             auto it_copy = q1.begin();
            //             idx_num=v_d[4];
            //             if(v_d[0]==8){
            //               print_lw(row,it_copy);
            ...
            //          }
            //      }else{
            //         if(counter==(2*ROW_ACCESS_DELAY+COL_ACCESS_DELAY)){
            //             int tmp1=-1;
            //            for(int i=0;i<1024;i++){
            //              if(!queueArray[i].empty()){
            //                  choice =false;
            ...
            if(registers[DRAM[(i+1)/1024][(i+1)%1024]] != registers[DRAM[(i+2)/1024][(i+2)%1024]]){
                printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,5,numberOfClockCycles,i,"bne","Nothing ","Nothing ","Nothing");
                i = DRAM[(i+3)/1024][(i+3)%1024];
               if(i%4!=0 || i<0 || i>=x){
                    cout<<"Invalid Memory Excess\n";
                    return 1;
        ...
        case 6:  //slt
            ...
            printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,6,numberOfClockCycles,i,"slt",Register_map[DRAM[(i+1)/1024][(i+1)%1024]]+" = "+ to_string(registers[DRAM[(i+1)/1024][(i+1)%1024]] ),"Nothing ","Nothing");
            i=i+4;
            // printRegisters(numberOfClockCycles);
            break;
        ...
        case 7: // jump 
            ...
            freqOfCommand[6]++;
            counter++;
            printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,7,numberOfClockCycles,i,"j","Nothing ","Nothing ","Nothing");
        ...
        case 8:  // lw
            ...
            printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,8,numberOfClockCycles,i,"lw","Nothing","Nothing","DRAM request issued");
        ...
        case 9:  // sw
            ...
            printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,9,numberOfClockCycles,i,"sw","Nothing","Nothing","DRAM request issued");
           if(check_memory_address(memory_address,cnt1,x)){
                    return 1;  //terminate the program
            }else{
        ...
        case 10:  //addi
            ...
            // printRegisters(numberOfClockCycles);
            printChange_in_Each_Cycle(ROW_ACCESS_DELAY,COL_ACCESS_DELAY,0,10,numberOfClockCycles,i,"addi",Register_map[DRAM[(i+1)/1024][(i+1)%1024]]+" = "+ to_string(registers[DRAM[(i+1)/1024][(i+1)%1024]] ),"Nothing ","Nothing");
            i = i+4;
            break;
